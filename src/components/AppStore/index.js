...
class AppStore extends Component {
  state = {activeTabId: tabsList[0].tabId, searhInput: ''}

  getFilteredApps = () => {
    const {activeTabId, searhInput} = this.state

    const filteredApps = appsList.filter(
      eachAppDetails =>
        eachAppDetails.category === activeTabId &&
        eachAppDetails.appName.toLowerCase().includes(searhInput.toLowerCase()),
    )
    return filteredApps
  }

  onChangeSearhInput = event => {
    this.setState({searhInput: event.target.value})
  }
      ...
      <div className="bg-contaiener">
        ...
        <div className="search-container">
          <input
            ...
            onChange={this.onChangeSearhInput}
            value={searhInput}
          />
          <img
